...
Sha256::Sha256() : state_{0x6a09e667,0xbb67ae85,0x3c6ef372,0xa54ff53a,0x510e527f,0x9b05688c,0x1f83d9ab,0x5be0cd19}, bit_len_(0) {}

void Sha256::update(std::span<const std::uint8_t> bytes) {
  bit_len_ += bytes.size() * 8;
...
std::array<std::uint8_t, 32> Sha256::digest() {
  auto total_bits = bit_len_;
  buffer_.push_back(0x80);
  while (buffer_.size() % 64 != 56) buffer_.push_back(0);
  for (int i = 7; i >= 0; --i) buffer_.push_back(static_cast<std::uint8_t>((total_bits >> (i * 8)) & 0xff));
  while (!buffer_.empty()) {
    transform(buffer_.data());
    buffer_.erase(buffer_.begin(), buffer_.begin() + 64);
  }
...
std::string Sha256::hex(std::span<const std::uint8_t> bytes) {
  Sha256 sha;
  sha.update(bytes);
  auto d = sha.digest();
  std::ostringstream out;
  for (auto b : d) out << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(b);
  return out.str();
}

std::string Sha256::hex(const std::string& bytes) {
  return hex(std::span<const std::uint8_t>(reinterpret_cast<const std::uint8_t*>(bytes.data()), bytes.size()));
}
